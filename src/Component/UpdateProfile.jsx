...
const UpdateProfile = () => {
    ...
    const handleSubmit = () =>{
        ...
        if(localStorage.getItem("password") == currentPassword){
            ...
            alert("You password is successfully updated")
  ...
  return (
    ...
    <div className='flex flex-col justify-center w-[300px] items-center '>
      <h1 className='text-[27px] pb-3 font-semibold'>Update Password</h1>
      
      <div type='text' className="pl-3 w-full outline-none bg-gray-200 focus:shadow-shadow3 rounded-xl flex items-center h-[2.5rem] mb-2"  placeholder='abc@example.com'>
        {localStorage.getItem("email")}
      </div>

      <label className='pb-2'>Current Password</label>
      <input type='password' className='mb-2 pl-2 w-full focus:shadow-shadow3 outline-none rounded-xl h-[2.5rem]' value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} />
      
      <label className='pb-2'>Password</label>
      <input type='password' className='mb-2 pl-2 w-full focus:shadow-shadow3 outline-none rounded-xl h-[2.5rem]' value={password} onChange={(e) => setPassword(e.target.value)} />
      
      <label className="pb-2">Confirm Password</label>
      <div className='relative w-full pb-3'>
      <input type={`${showPassword ? "text" : "password"}`} className="mb-2 pl-2 w-full focus:shadow-shadow3 outline-none rounded-xl h-[2.5rem]"  value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)}/>
      ...
      <div>
        <button className='bg-gray-700 opacity-50 rounded-3xl w-[13rem] h-[3rem] text-color3' onClick={handleSubmit}> Update</button>
      </div>
    </div>
    </div>
  )
